...
import { createWeatherDashboardLoader } from "../js/app.js?v=1.5.2-location-sync-test";
...
test("les revisions JavaScript, CSS et PWA restent coherentes", () => {
    assert.match(INDEX_SOURCE, /js\/app\.js\?v=1\.5\.2-location-sync/);
    assert.match(APP_SOURCE, /components\/favorites\.js\?v=1\.5\.2-location-sync/);
    assert.match(APP_SOURCE, /components\/hourly-forecast\.js\?v=1\.5\.2-location-sync/);
    assert.match(SW_SOURCE, /const CACHE_PREFIX = "meteosignal-static"/);
    assert.match(SW_SOURCE, /const DEPLOYMENT_REVISION = `\$\{APP_VERSION\}-location-sync`/);
    assert.match(SW_SOURCE, /const CACHE_VERSION = `v\$\{DEPLOYMENT_REVISION\}`/);
    assert.match(INDEX_SOURCE, /css\/[^"']+\?v=1\.5\.2-location-sync/);
